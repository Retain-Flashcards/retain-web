...
export default function useNotes(deckId) {
    ...
    const fetchDeckNotes = async (page) => {
        const PAGE_SIZE = 20
        const { data, error, count } = await supabase.from('notes').select('*', { count: 'exact' }).range(page * PAGE_SIZE, (page + 1) * PAGE_SIZE - 1).eq('deck_id', deckId).eq('active', true).order('created_at', { ascending: false })
    ...
    const deleteNote = async (noteId) => {
        //TODO: Maybe replace RPC with cascade delete?
    ...
    const resetNote = async (noteId) => {
        //TODO: Update reset_note RPC with new card tracking
